...
def calculate_lectures_in_period(start_date, end_date):
    total = 0
    current = start_date
...
def find_date_for_lectures(start_date, lectures_needed):
    total = 0
    current = start_date
...
def main_web(attended, conducted, deadline, today, skip_start=None, skip_end=None, skipped_lectures=0):
    results = {
    ...
    for t in [50, 60, 70, 75]:
        ...
        if additional_skip and additional_skip > 0:
            ...
            new_skip_end_date = find_date_for_lectures(baseline, additional_skip)
